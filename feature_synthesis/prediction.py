...
def prediction(data_file, target):
    ...
    # 预测S
    model = torch.load("pth/useful_6.pth", map_location=torch.device('cpu'))
    # model = torch.load("./pth/S_model_490_15.5063.pth")
    dataset_test = Dataset(data_file, 0, target)
    ...
    return sp_MAE, sp_prediction,label

if __name__ == "__main__":
    data_file = "../data/test_all_features_norm.npy"
    prediction_file = '../data/prediction.npy'

    sp_MAE, sp_prediction,label = prediction(data_file,"../data/test_without_resp.npy")
    ...
    plot_2vectors(label, sp_prediction, 'sp')
